...
let query = confirm(question);//Задаем вопрос пользователю;
if (query) {//При истинном условии т.е. при нажатии ОК;
    let answer = 'Вы будущий кодер!';//Присваиваем переменной ответ "Да";
    alert(answer);//Выводим ответ;
}
else {//При ложном условии т.е. при нажатии Cancel;
    let answer = 'Похоже Вы дизайнер';//Присваиваем переменной ответ "Нет";
    alert(answer);//Выводим ответ;
}
}
}
...
if (login==='admin'){//Проверка логина на корректность;
    ...
    if (password ==='qwerty'){//Вложенный if для проверки пароля;
...
let message = (userValue === comuterValue) ? 'Ничья' : 
(userValue === 'камень' && comuterValue === 'колодец'||comuterValue === 'бумага') ? 'Ты проиграл' : 
(userValue ==='камень'&& comuterValue==='ножницы') ? 'Ты выиграл': (userValue ==='ножницы' && comuterValue ==='колодец'||comuterValue === 'камень') ? 
'Ты проиграл' : (userValue === 'ножницы'&& comuterValue === 'бумага') ? 'Ты выиграл' : (userValue === 'бумага' && comuterValue === 'ножницы') ? 
('ножницы'||comuterValue==='камень') : 'Ты выиграл' ? 
(userValue === 'колодец' && comuterValue==='бумага') : 'Ты проиграл';
  alert(message);
}
}
}
