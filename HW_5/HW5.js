...
for (item of arr) {//Перебираем циклом каждый элемент массива;
...
let str = prompt('Введите строку');//ввод строки;
const arr = str.split(' ');//разбиваем строку по пробелам;
const intersection = arr.filter((item) => forbidden.includes(item));//Фильтруем массив на наличие запрещенных строк;
let result = arr.join(' ');//склеиваем строку;
alert(result);//выводим результат;
alert(intersection.join(' '));//выводим строку под запретом;
}
//27
{
const forbidden = ['хутин', 'рф', 'че', 'ВК', 'одноклассники', 'Яндекс'];//Декларируем массив запрещенных строк;
...
let   str = "<select>";
for (let currency of currencies){
str += currencies.reduce(currencies =>`<option>${currency}</option>`);//Реализуем задачу For Select Option используя reduce;
}
str+= "</select>";
document.write(str);
}
